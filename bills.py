# TODO: Variable names still mostly shit. Take another look when user input.
class PayDay:
...
# Can be used to count payday or bill objects in a list.
# Can be used for both due to the way objects are implemented.
...
if left_over < 0:
    ...
    # Find the "middle" range. This range will be covered by the first payday.
    middle = range(first_payday, second_payday)

    # Create a list for each pay period
    first_pay_period = []
    second_pay_period = []

    # Checks if a bills due date falls under the "middle" period.
    def separate_bills():
        for _ in bills:
            if _.date in middle:
                first_pay_period.append(_)
            else:
                second_pay_period.append(_)
